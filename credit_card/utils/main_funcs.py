...
# 예측모델
CSV_FILEPATH = os.path.join(os.getcwd(), 'credit_card', 'BankChurners.csv')
...
def predict_card(username):
    ...
    ex = {'Customer_Age':[0, user.age], 'Gender':[0, user.gender], 'Dependent_count':[0, user.dependent], 'Education_Level':[0, user.Edu_Level], 'Marital_Status':[0, user.marige], 'Income_Category':[0, user.income]}
    ex = pd.DataFrame(data=ex)
    prediction = Ensemble_pipe.predict(ex)[1]
